...
while (( attempt <= MAX_RESTARTS )); do
    ...
    if (( exit_code == 1 )); then
        echo "[$(date --iso-8601=seconds)] Fatal error. Stopping supervisor. Log: $log_file" >&2
        exit "$exit_code"
    fi

    if (( exit_code == 120 || exit_code == 130 || exit_code == 143 )); then
        echo "[$(date --iso-8601=seconds)] Interrupted. Stopping supervisor. Log: $log_file" >&2
        exit "$exit_code"
    fi
    ...
    echo "[$(date --iso-8601=seconds)] Sleeping ${sleep_s}s before restart. Log: $log_file"
    sleep "$sleep_s"
